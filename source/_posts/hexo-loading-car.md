---
title: Hexo + Butterfly 更换加载动画
date: 2023-03-03 17:00:00
tags:
  - Hexo
categories: Blog
---
> 参考文章：[https://blog.zhheo.com/p/32776e99.html](https://blog.zhheo.com/p/32776e99.html)

## 资源文件

[CodePen](https://codepen.io/abxlfazl)

## 配置

1. 在主题配置文件`_config.butterfly.yml`中，找到`preloader`并设置`enable`为`true`

2. 在 `\themes\butterfly\source\css`目录下新建 `loading_car.css`文件，添加：
...
3. 在主题配置文件 `_config.butterfly.yml`的 `inject`，在 `head`直接引入 `css`文件
    ```yaml
    - <link rel="stylesheet" href="/css/loading_car.css">
    ```

4. 打开`\themes\butterfly\layout\includes\loading\fullpage-loading.pug`文件，修改成：
    ```css
    #loading-box
      .loading-left-bg
      .loading-right-bg
